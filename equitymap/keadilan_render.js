...
function build(){
 ...
 /* ---- 3b ekuitas vertikal tingkat kabupaten (hanya bila input BPS kabupaten tersedia) */
 const SK=K.sosek,skEl=$('kdSosek');
 if(SK&&skEl){
  skEl.hidden=false;
  skEl.innerHTML=`<div class="ct" style="margin-top:14px">Tingkat kabupaten (BPS: kemiskinan, IPM, pengeluaran per kapita — ${SK.n} kab/kota, ${SK.pop_pct}% penduduk)</div>
   <div class="kv" id="kdKv3b"></div>
   <div class="grid2"><div><div class="ct">Kurva konsentrasi — kabupaten diurut miskin → kaya</div><div style="height:300px"><canvas id="cKdConcKab"></canvas></div></div>
   <div><div class="ct">Kuintil kabupaten menurut pengeluaran per kapita (tertimbang penduduk)</div><div style="height:300px"><canvas id="cKdQuintKab"></canvas></div></div></div>
   <div class="grid2"><div class="tablewrap" style="max-height:230px"><table id="tblKdQuintKab"><thead><tr><th>Kuintil</th><th>Kab/kota</th><th>Penduduk</th><th>Pengeluaran/kapita (rb Rp)</th><th>Kemiskinan %</th><th>IPM</th><th>Charger /100 rb</th><th>≤10 km %</th><th>&gt;25 km %</th><th>Tanpa charger</th></tr></thead><tbody></tbody></table></div>
   <div class="tablewrap" style="max-height:230px"><table id="tblKdProvCi"><thead><tr><th>Provinsi (≥8 kab)</th><th>n</th><th>CI ~ pengeluaran</th><th>CI ~ IPM</th><th>CI ~ kemiskinan</th></tr></thead><tbody></tbody></table></div></div>
   <div class="why" id="kdWhy3b"></div>`;
  $('kdKv3b').innerHTML=kv([[SK.ci.expend,'CI charger ~ pengeluaran/kapita kabupaten','g'],[SK.ci.hdi,'CI charger ~ IPM kabupaten'],[SK.ci.poverty,'CI charger ~ kemiskinan (miskin→kaya)'],[SK.ci.access_expend,'CI akses ≤10 km ~ pengeluaran'],[SK.rho.expend,'ρ Spearman charger/kapita ~ pengeluaran']]);
  const SC=SK.curves;
  mk('cKdConcKab',{type:'line',data:{datasets:[{label:'~ pengeluaran/kapita (CI '+SK.ci.expend+')',data:pts(SC.expend),borderColor:NAVY,pointRadius:0,borderWidth:2,tension:.1},{label:'~ IPM (CI '+SK.ci.hdi+')',data:pts(SC.hdi),borderColor:RED,pointRadius:0,borderWidth:2,tension:.1},{label:'~ kemiskinan (CI '+SK.ci.poverty+')',data:pts(SC.poverty),borderColor:ORANGE,pointRadius:0,borderWidth:1.8,tension:.1},{label:'akses ≤10 km ~ pengeluaran (CI '+SK.ci.access_expend+')',data:pts(SC.access_expend),borderColor:GREEN,pointRadius:0,borderWidth:1.8,tension:.1},diag()]},
   options:{maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{boxWidth:10,font:{size:10}}}},scales:{x:unit01('kumulatif penduduk, kabupaten termiskin → terkaya'),y:unit01('kumulatif charger')}}});
  const KQ=SK.quint;
  mk('cKdQuintKab',{data:{labels:KQ.map(q=>'Q'+q.q+' · Rp '+fmt(q.expend)+' rb'),datasets:[{type:'bar',label:'charger /100 rb',data:KQ.map(q=>q.per100k),backgroundColor:NAVY,yAxisID:'y',borderRadius:3},{type:'line',label:'≤10 km %',data:KQ.map(q=>q.within10),borderColor:GREEN,backgroundColor:GREEN,pointRadius:4,yAxisID:'y2'},{type:'line',label:'>25 km %',data:KQ.map(q=>q.beyond25),borderColor:RED,backgroundColor:RED,pointRadius:4,yAxisID:'y2'}]},
   options:{maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{boxWidth:10,font:{size:10}}}},scales:{y:yAx('charger /100 rb'),y2:{position:'right',beginAtZero:true,max:100,grid:{display:false},title:{display:true,text:'% penduduk',font:{size:10}},ticks:{font:{size:10}}},x:xAx('kuintil kabupaten termiskin → terkaya')}}});
  $('tblKdQuintKab').querySelector('tbody').innerHTML=KQ.map(q=>`<tr><td><b>Q${q.q}</b></td><td class="n">${q.n}</td><td class="n">${fmt(q.pop)}</td><td class="n">${fmt(q.expend)}</td><td class="n">${f1(q.poverty)}</td><td class="n">${f1(q.hdi)}</td><td class="n">${f2(q.per100k)}</td><td class="n">${f1(q.within10)}</td><td class="n">${f1(q.beyond25)}</td><td class="n">${q.zero}</td></tr>`).join('');
  $('tblKdProvCi').querySelector('tbody').innerHTML=SK.prov_ci.map(p=>`<tr><td><b>${esc(p.name)}</b></td><td class="n">${p.n}</td><td class="n" style="color:${p.ci_expend>0.1?RED:p.ci_expend<-0.1?GREEN:'inherit'}">${p.ci_expend}</td><td class="n">${p.ci_hdi}</td><td class="n">${p.ci_poverty}</td></tr>`).join('');
  const kq1=KQ[0],kq5=KQ[KQ.length-1];
  $('kdWhy3b').innerHTML=`<b>Yang ditambahkan data kabupaten.</b> CI terhadap pengeluaran per kapita kabupaten ${SK.ci.expend} (provinsi: ${CI.grdp} terhadap PDRB) dan terhadap IPM ${SK.ci.hdi} (provinsi ${CI.hdi}): ${Math.abs(SK.ci.expend)>Math.abs(CI.grdp)?'ketidakadilan vertikal <b>lebih besar</b> di dalam provinsi daripada yang terlihat antar-provinsi':'ketidakadilan vertikal antar-kabupaten <b>tidak lebih besar</b> daripada antar-provinsi'} — kuintil kabupaten termiskin (${kq1.n} kab, ${jt(kq1.pop)} jiwa, pengeluaran Rp ${fmt(kq1.expend)} rb, kemiskinan ${f1(kq1.poverty)}%) mendapat ${f2(kq1.per100k)} charger/100 rb dan ${f1(kq1.within10)}% ≤10 km, kuintil terkaya ${f2(kq5.per100k)} dan ${f1(kq5.within10)}% (${(kq5.per100k/Math.max(kq1.per100k,0.01)).toFixed(1)}×). CI dalam-provinsi tertinggi: ${SK.prov_ci.slice(0,3).map(p=>esc(p.name)+' '+p.ci_expend).join(', ')}; terendah (paling pro-miskin): ${SK.prov_ci.slice(-2).map(p=>esc(p.name)+' '+p.ci_expend).join(', ')}. Sumber: tabel BPS kabupaten/kota yang dibaca <code>equitymap/sosek.py</code>.`;
 }
 ...
 if(K.dyn&&window.buildKdDyn){try{window.buildKdDyn(K.dyn,K);}catch(e){console.error('kd dyn',e);}}
