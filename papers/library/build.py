...
PAPERS = [
    ...
    dict(
        id="bacaan-evmap", n=18, category="akses", tags=["akses", "bisnis"],
        title="Intisari bacaan — EV Equity Roadmap (UC Berkeley CLEE): peta prioritas × kelayakan untuk penempatan charger yang adil, dan apa yang bisa dipakai untuk Indonesia",
        short="Intisari bacaan: EV Equity Roadmap (Berkeley) → Peta Ekuitas Indonesia",
        kind="Intisari bacaan", venue="Bahan bacaan — bukan untuk publikasi",
        alt="Dipakai Paper 1, Paper 2, CUPUM Bab 1, RQ4", venue_src="—",
        status="bacaan", stage="Intisari selesai; butir [VERIFY] menunggu cek ke halaman asli (situs diblokir dari lingkungan bangun)", pct=100,
        target="Rujukan hidup — perbarui bila CLEE merilis versi baru", lead="Qashtalani Haramaini",
        goal="Merangkum EV Equity Roadmap (evmap.climateplans.org) cukup rinci untuk dijadikan cetak biru — dua lapisan prioritas × kelayakan pada piksel 100 m per yurisdiksi — lalu memetakan lapisan demi lapisan ke data yang ada di repositori ini, dan menandai mana yang bisa dipakai sekarang, mana yang butuh data tambahan, dan mana yang tidak sebaiknya ditiru.",
        finding="Tiga hal dipakai langsung: pemisahan prioritas dari kelayakan sebagai dua sumbu; kriteria & bobot sebagai keluaran yang terlihat; akses = jarak ke charger yang ada, dinormalkan di dalam yurisdiksi. Tiga lapisan tidak punya padanan data publik Indonesia (CalEnviroScreen, hosting-capacity utilitas, penyewa/hunian multi-keluarga) dan diganti proksi provinsi/GI/kepadatan di tab Peta Ekuitas — 63,7 % penduduk Indonesia tinggal ≤10 km dari SPKLU operasional, 12,0 % lebih dari 25 km.",
        data=["EV Equity Roadmap (CLEE UC Berkeley, 2024) — piksel 100 m, seluruh California",
              "Padanan Indonesia: Kontur Population 2023 (H3), geoBoundaries ADM2, master SPKLU nasional, data/grid-id, BPS provinsi"],
        method=["Pembacaan prinsip pertama: mengapa dua sumbu, mengapa normalisasi per yurisdiksi",
                "Tabel transfer lapisan → padanan data Indonesia → status di repositori",
                "Batas & kritik untuk disitir; format sitasi"],
        tabs=["ekuitas", "equity", "locint"],
        html=md_to_html(rd("papers/bacaan_evmap_equity_roadmap.md")),
        files=[["Intisari (.md)", "papers/bacaan_evmap_equity_roadmap.md"],
               ["Pipeline Peta Ekuitas (.py)", "equitymap/prepare.py"]],
        todo=["Cek butir [VERIFY] langsung ke evmap.climateplans.org dan halaman CLEE (dari jaringan yang tidak diblokir)",
              "Tambahkan indikator BPS tingkat kabupaten (kemiskinan, IPM) ke Peta Ekuitas",
              "Tambahkan POI OSM sebagai lapisan sumber daya komunitas",
              "Ganti jarak garis lurus dengan waktu tempuh OSRM"],
    ),
]
...
PAPERS.append(dict(
    ...
    finding="Akses (Gini 0,274) dan kepemilikan (Gini 0,595) adalah dua hal berbeda; ketidaksetaraan terstruktur — 63 % antar-provinsi (Theil), kota 5,0× kabupaten per kapita, 100 kabupaten (15,6 jt jiwa) tanpa charger, rasio 20:20 = 33,7. Secara vertikal pro-kaya menurut IPM (CI 0,365) tetapi bukan menurut PDRB (0,212): gradiennya kepadatan & pembangunan manusia. Kesetaraan per kapita menuntut +2.097 charger (+44 %). Ekuitas nyaris gratis: bobot 2× untuk provinsi termiskin mengorbankan 0,09 poin cakupan nasional dan menambah 3,8 poin bagi kuintil termiskin. Lanjutan: ketimpangan mengelompok di ruang (Moran's I 0,385; 28 kab HH Jabodetabek–Bali vs 49 kab LL, separuhnya Papua); diproduksi ulang tiap tahun (PLN menurunkan Gini 0,887 → 0,471 pada 2021–2024, lalu 0,506; mitra non-PLN menaikkannya ke 0,595 sambil menambah cakupan 1 poin); harga pemerataan: Gini 0,4 = +935 charger, 0,3 = +1.793, 0,2 = +3.330; pada 1.000 situs pemerataan penuh berbiaya 3,7 poin cakupan untuk 0,10 Gini, dan aturan cakupan justru melayani kuintil termiskin lebih cepat daripada aturan per kapita.",
    ...
    method=["Lorenz/Gini (charger, kW, situs, akses, heksagon); dekomposisi Theil antar/dalam provinsi dan kota/kabupaten",
            ...
            "Moran's I & LISA (bobot ketetanggaan geoBoundaries ADM2, 999 permutasi); deret waktu Jawa Barat (tanggal nyata) dan nasional PLN (ID SPKLU dikalibrasi, akurasi 93 %)",
            "Skenario water-filling menuju target Gini; simulasi campuran aturan pemerataan × cakupan pada 250–2.000 situs"],
    tabs=["keadilan", "ekuitas", "equity"],
    html=md_to_html(rd("papers/catatan_ekuitas_kesetaraan.md")),
    files=[["Catatan (.md)", "papers/catatan_ekuitas_kesetaraan.md"], ["Pipeline (.py)", "equitymap/keadilan.py"], ["Lanjutan spasial · waktu · skenario (.py)", "equitymap/dinamika.py"]],
    todo=["Tambahkan indikator BPS tingkat kabupaten untuk CI dalam-provinsi (pembaca equitymap/sosek.py sudah siap, menunggu tabel BPS)",
          "Ganti radius heksagon dengan waktu tempuh jalan pada kurva cakupan",
          "Masukkan biaya sambungan jaringan per situs ke aturan penempatan (zona 'jaringan lemah') dan verifikasi asumsi biaya per charger ke RAB PLN",
          "Angkat §6–§9 (uji aturan penempatan, struktur spasial, dinamika, skenario) menjadi bagian hasil Paper 2 / bab sintesis RQ4"],
))
